...
namespace CSLERP.DBData
{
    ...
    class SJVDB
    {
        public List<SJVHeader> getFilteredSJVHeader(string userList, int opt, string userCommentStatusString)
        {
            SJVHeader jvh;
            List<SJVHeader> JVHeaders = new List<SJVHeader>();
            try
            {
        ...
        public static List<SJVDetail> getSJVDetail(SJVHeader jvh)
        {
            SJVDetail jvd;
            List<SJVDetail> VDetail = new List<SJVDetail>();
            try
            {
                string query = "";
                SqlConnection conn = new SqlConnection(Login.connString);
                query = "select a.RowID,a.DocumentID,a.TemporaryNo, a.TemporaryDate,a.AccountCode,b.Name,a.AmountDebit,a.AmountCredit,"+
                  ...
                  " from SJVDetail a LEFT OUTER JOIN  " +
        ...
        public Boolean reverseSJVHeader(SJVHeader jvh)
        {
            Boolean status = true;
            string utString = "";
            try
        ...
        public Boolean updateSJVHeaderAndDetail(SJVHeader jvh, SJVHeader prevjvh, List<SJVDetail> JVDetails)
        {
            Boolean status = true;
            string utString = "";
            try
            {
                ...
                utString = utString +
                    ...
                    Main.QueryDelimiter;
                foreach (SJVDetail jvd in JVDetails)
                {
        ...
        public static SJVHeader getSJVHeaderForTrialBalance(SJVHeader jvhTemp)
        {
            SJVHeader jvh = new SJVHeader();
            try
            {
        ...
        public static SJVHeader getSJVHeaderPerInvoiceOut(invoiceoutheader ioh)
        {
            SJVHeader pjvh = new SJVHeader();
            try
            {
                string query = "select RowID, DocumentID,TemporaryNo,TemporaryDate,JournalNo,JournalDate," +
                    ...
                    " from SJVHeader" +
                    " where INVDocumentID = '"+ ioh.DocumentID +"' and InvTempNo = " + ioh.TemporaryNo +
        ...
        public static Boolean UpdateSJVHeaderDetailDuringApproveInvoiceIN(List<SJVDetail> SJVDetail, SJVHeader pjvh)
        {
            //call when invoice inward is saved
            //Invoice types MRN,Work Order, PO General
            ...
            try
            {
                updateSQL = "Delete from SJVDetail where DocumentID='" + pjvh.DocumentID + "'" +
                    " and TemporaryNO=" + pjvh.TemporaryNo +
                    " and TemporaryDate='" + pjvh.TemporaryDate.ToString("yyyy-MM-dd") + "'";
                utString = utString + updateSQL + Main.QueryDelimiter;
                //--
                foreach (SJVDetail jvd in SJVDetail)
                {
                    updateSQL = "insert into SJVDetail " +
                            "(DocumentID,TemporaryNo,TemporaryDate,AccountCode,AmountDebit,AmountCredit,SLCode,SLType,INVDocumentID,InvTempNo,InvTempDate) " +
                                "values ('" + jvd.DocumentID + "'," +
